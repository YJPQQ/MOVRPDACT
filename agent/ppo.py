...
from agent.utils import validate_with_construct
from construct_model.TSP.test_n20 import construct_routes_TSP20
from construct_model.TSP.test_n50 import construct_routes_TSP50
from construct_model.TSP.test_n100 import construct_routes_TSP100
...
class PPO:
    ...
    def rollout_with_construct(self, problem, val_m, batch, pref, construct_epoch,do_sample = False, record = False, show_bar = False):
        ...
        batch_feature = problem.input_feature_encoding(batch)
            ...
            # initial solution WITH construct method before
        if(self.opts.graph_size == 20):
            routes = construct_routes_TSP20(batch_feature, pref, construct_epoch)
        elif(self.opts.graph_size == 50):
            routes = construct_routes_TSP50(batch_feature, pref, construct_epoch)
        elif(self.opts.graph_size == 100):
            routes = construct_routes_TSP100(batch_feature, pref, construct_epoch)    
        
        # get initial solutions
        solutions = move_to(problem.get_initial_solutions_with_construct(batch, routes), self.opts.device).long()
        # get initial cost
        objs = problem.get_costs(batch, solutions)
    ...
    def start_inference(self, problem, val_dataset, tb_logger):
        if self.opts.distributed:            
            ...
            if self.opts.with_construct_before:
                mp.spawn(validate, nprocs=self.opts.world_size, args=(problem, self, val_dataset, tb_logger, self.opts.construct_begin_epoch,  True))
            else:
                mp.spawn(validate, nprocs=self.opts.world_size, args=(problem, self, val_dataset, tb_logger, True))
        else:
            if self.opts.with_construct_before:
                validate_with_construct(0, problem, self, val_dataset, tb_logger, self.opts.construct_begin_epoch, distributed = False)
            else:
    ...
    def start_training(self, problem, val_dataset, tb_logger):
        if self.opts.distributed:
            if self.opts.with_construct_before:
                mp.spawn(train_with_constrcut, nprocs=self.opts.world_size, args=(problem, self, val_dataset, tb_logger))
            else:
                mp.spawn(train, nprocs=self.opts.world_size, args=(problem, self, val_dataset, tb_logger))
        else:
            if self.opts.with_construct_before:
                train_with_constrcut(0, problem, self, val_dataset, tb_logger)
            else:
...
def train_with_constrcut(rank, problem, agent, val_dataset, tb_logger):
    ...
    construct_begin_epoch = opts.construct_begin_epoch - 1
    construct_end_epoch = opts.construct_end_epoch
    construct_epoch = construct_begin_epoch
    epochs_each_construct_epoch_to_learn = opts.epochs_each_construct_epoch_to_learn
    
    # Start the actual training loop
    for epoch in range(opts.epoch_start, opts.epoch_end):
        
        if epoch % epochs_each_construct_epoch_to_learn == 0:
            construct_epoch = construct_epoch + 1
        ...
        pbar = tqdm(total = (opts.K_epochs) * (opts.epoch_size // opts.batch_size) * (opts.T_train // opts.n_step) ,
                    ...
                    bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}')
        for batch_id, batch in enumerate(training_dataloader):
            train_batch_with_construct(rank,
                        problem,
                        agent,
                        epoch,
                        step,
        ...
        if rank == 0: 
            validate_interval = opts.validate_interval
            if ((epoch == 0) or ((epoch + 1) % validate_interval == 0)):
                validate_with_construct(rank, problem, agent, val_dataset, tb_logger, construct_epoch, _id = epoch)
...
def train_batch_with_construct(
        rank,
        problem,
        agent,
        epoch,
    ...
    solution = move_to_cuda(problem.get_initial_solutions_with_construct(batch,routes),rank) if opts.distributed \
                        else move_to(problem.get_initial_solutions_with_construct(batch,routes), opts.device)
    objs = problem.get_costs(batch, solution)
