...
class PPO:
    ...
    def rollout_with_construct(self, problem, val_m, batch, pref, do_sample = False, record = False, show_bar = False):
        ...
        if(self.opts.graph_size == 20):
            routes = construct_routes_TSP20(batch_feature, pref)
        elif(self.opts.graph_size == 50):
            routes = construct_routes_TSP50(batch_feature, pref)
...
def train_with_constrcut(rank, problem, agent, val_dataset, tb_logger):
    ...
    for epoch in range(opts.epoch_start, opts.epoch_end):
        ...
        if rank == 0: 
            if epoch % 5 == 0:
                validate_with_construct(rank, problem, agent, val_dataset, tb_logger, _id = epoch)
...
def train_batch(
    ...
    while t < T:
        ...
        for _k in range(K_epochs):
            ...
            # get next value
            R = agent.critic(agent.actor(problem,batch_feature_pref,solution,action,only_critic = True))[0]
