...
class TSP(object):
    ...
    def __init__(self, p_size, init_val_met = 'greedy', with_assert = False, step_method = '2_opt',dec_method = 'TCH' ,P = 10, DUMMY_RATE = 0):
    ...
    def get_initial_solutions_with_construct(self,batch,routes):
        batch_size = batch['coordinates'].size(0)
        
        set = torch.tensor(routes,dtype=torch.int64)
        rec = torch.zeros(batch_size, self.size).long()
        index = torch.zeros(batch_size,1).long()
    ...
    def get_costs(self, batch, rec):
        ...
        d2 = batch['coordinates'][:,:,:2]
        
        d21 = batch['coordinates'][:,:,2:].gather(1, rec.long().unsqueeze(-1).expand(batch_size, size, 2))
        d22 = batch['coordinates'][:,:,2:]
        ...
        length2 =  (d21  - d22).norm(p=2, dim=2).sum(1)
        
        length = torch.cat((length1[:,None], length2[:,None]), dim = 1)
...
class TSPDataset(Dataset):
    def __init__(self, filename=None, size=20, num_samples=10000, offset=0, distribution=None, DUMMY_RATE=None):
        ...
        if filename is not None:
            assert os.path.splitext(filename)[1] == '.pt', 'file name error'
            ...
            self.data = [{'coordinates': torch.FloatTensor(self.size, 4).uniform_(0, 1)} for i in range(num_samples)]
